import { g as t, D as u, w as m, S as p, C as d, b as x } from "./index.js";
class P extends u {
	init() {
		(this.DOM = {
			heading1: this.block.querySelector(".name__animate"),
			heading2: this.block.querySelector(".name__anything"),
			button: this.block.querySelector(".name__button"),
			squiggle: this.block.querySelector("#name-squiggle"),
		}),
			(this.selector = {
				animate: t.utils.selector(this.DOM.heading1),
				anything: t.utils.selector(this.DOM.heading2),
			}),
			(this.defaults = { ease: "power2.out", duration: 0.6 }),
			this.createTimeline(),
			...
			window.addEventListener("mousemove", (l) => {
				const c = t.utils.mapRange(0, e, -20, 20, l.x),
					...
					w = t.utils.clamp(
						-1,
						1,
						t.utils.mapRange(e * 0.25, e * 0.75, 1, -1, l.x)
					);
				i(f * 1 * w), o(c), a(f);
			});
	}
	char1() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.animate(".a > span"),
			s = this.selector.animate(".a > span > span");
		return (
			e.set(r, { transformOrigin: "50% 100%" }),
			e.from(s, { yPercent: 100 }),
			e.from(
				r,
	...
	char2() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.animate(".name__flair--circles"),
			s = this.selector.animate(".name__flair--circles svg"),
			o = this.selector.animate(".name__flair--windmill"),
			a = this.selector.animate(".n > span > span"),
			i = this.selector.animate(".n > span > span > span");
		return (
			e.set(i[1], { autoAlpha: 1 }),
			e.set(r, { autoAlpha: 1, yPercent: 100 }),
			e.from(s, { scale: 0, ease: "back.out(1.7)" }),
			e.to(r, {
				yPercent: -200,
				autoAlpha: 0,
				duration: 1.5,
			...
			e.from(a, { yPercent: 100, duration: 0.4 }, "<"),
			e.from(
				o,
				{
			...
			e.from(i[0], { rotationY: -180, duration: 0.3 }, "+=.4"),
			e.to(i[1], { rotationY: 180, duration: 0.3 }, "<"),
			e.to(
				o,
				{ rotationZ: 90, duration: 0.3, repeat: -1, repeatDelay: 1 },
				"<"
			),
			e
		);
	...
	char4() {
		...
		return t.from(e, { xPercent: -100, ...this.defaults });
	}
	char5() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.animate(".a2"),
			s = this.selector.animate(".name__flair--star"),
			o = this.selector.animate(".name__flair--star svg"),
			a = this.selector.animate(".a2 > span > span");
		return (
			e.set(s, { xPercent: -150, autoAlpha: 1 }),
			e.from(o, { scale: 0, duration: 0.4 }),
			e.add(this.char4(), "+=.6"),
			e.to(s, { xPercent: 0 }, "<"),
			e.set(r, { overflow: "hidden" }),
			e.to(s, { yPercent: 130, ease: "power2.in" }, "+=.5"),
			e.from(a, { yPercent: 100 }, "-=.3"),
			e.to(
				o,
	...
	char6() {
		...
		return (
			e.set(s, { autoAlpha: 1 }),
			e.from(o[0], { yPercent: 100, duration: 0.4 }, "<"),
			e.fromTo(
				[o[1], o[2]],
				{ yPercent: 100 },
				{
			...
			e.to(o[0], { yPercent: -100 }, "-=.6"),
			e.from(r, { yPercent: 100, duration: 0.9 }, "<"),
			e.add(this.char7(), "<"),
			e
		);
	}
	char7() {
		...
		return t.from(e, { yPercent: 100, duration: 0.9 });
	}
	char8to9() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.anything(
				".a span:first-of-type, .n span:first-of-type"
			),
			s = this.selector.anything(
	...
	char10() {
		...
		return t.from(e, { rotationY: -180, duration: 1, scale: 0 });
	}
	char11() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.anything(".name__flair--bolt"),
			s = this.selector.anything(".name__flair--bolt #bolt-path"),
			o = this.selector.anything(".name__flair--bolt #bolt-rect"),
			a = this.selector.anything(".t span");
		return (
			e.set(r, { autoAlpha: 1 }),
			e.from(s, { drawSVG: "0", duration: 1, ease: "power3.inOut" }),
			e.from(
				o,
				{
					yPercent: 100,
			...
			e.from(a, { scale: 0, ease: "back.out(1.4)" }, "<.5"),
			e
		);
	}
	char12() {
		const e = t.timeline({ defaults: this.defaults }),
			r = this.selector.anything(".name__flair--worm img"),
	...
	char13() {
		...
		return (
			e.from(r, { autoAlpha: 0, duration: 0.1 }, "<"),
			e.from(r, { rotationX: -450, duration: 1.3 }, "<.14"),
			e.add(this.char14(), "<+=.5"),
			e.to(
				r,
				{
	...
	char14() {
		...
		return e.from(r, { xPercent: -100 }), e;
	}
	char15() {
		const e = t.timeline({ defaults: this.defaults }),
	...
	createTimeline() {
		const e = t.timeline({ id: "name", defaults: this.defaults });
		e.set([this.DOM.heading1, this.DOM.heading2], { autoAlpha: 1 }),
			m(() => {
				e.add(this.char1()),
					e.add(this.char2(), 0.4),
					e.add(this.char3(), 1),
					e.add(this.char5(), 0.8),
...
class T extends u {
	init() {
		...
		(this.DOM = e), this.createTimelines();
	}
	staggerWord(e) {
		return t.to(e, {
			color: "#0ae448",
			ease: "power3.inOut",
			duration: 0.8,
			stagger: { each: 0.075, repeat: 1, yoyo: !0 },
		});
	}
	wordOne() {
		const r = this.DOM.highlightSections[0].querySelectorAll(
				".home-intro__highlight-word"
			),
			s = new n(r[0], { type: "chars" }),
			o = new n(r[1], { type: "chars" }),
			a = t.timeline();
		return (
			t.set(this.DOM.stem, { drawSVG: "0" }),
			t.set(this.DOM.flower, { scale: 0, transformOrigin: "50% 50%" }),
	...
	wordTwo() {
		const r = this.DOM.highlightSections[1].querySelectorAll(
				".home-intro__highlight-word"
			),
			s = new n(r[0], { type: "chars" }),
			o = new n(r[1], { type: "chars" }),
			a = new n(r[2], { type: "chars" }),
			i = t.timeline();
		return (
			i.add(this.staggerWord([s.chars, o.chars, a.chars])) +
				i.to(
					[s.chars, o.chars, a.chars],
					{
						rotateX: 360,
						duration: 1,
						stagger: 0.075,
	...
	wordThree() {
		const r = this.DOM.highlightSections[2].querySelectorAll(
				".home-intro__highlight-word"
			),
			s = new n(r[0], { type: "chars" }),
			o = new n(r[1], { type: "chars" }),
	...
	createTimelines() {
		const e = t.timeline({
			scrollTrigger: { trigger: this.block, start: "top 60%", once: !0 },
		});
		t.set(this.block, { autoAlpha: 1 }),
			t
				.matchMedia()
				.add(
					[
					...
					() => {
						e.add(this.wordOne()),
							...
							e.add(this.wordThree(), "-=3");
					}
				);
	}
}
t.registerPlugin(p, d);
class S extends u {
	init() {
		(this.selectors = {
			...
			choreograph: t.utils.selector(
				".home-animate__text-group--choreograph"
			),
			...
			(this.DOM = {
				...
				super: this.selectors.block(
					".home-animate__text-group--super"
				)[0],
				choreograph: this.selectors.block(
					".home-animate__text-group--choreograph"
				)[0],
			}),
			(this.triggerDefaults = { start: "left 70%", horizontal: !0 }),
	...
	introMobile() {
		const e = this.selectors.block(".home-animate__intro-heading"),
			...
			l = t.timeline({ scrollTrigger: { trigger: e, start: "top 70%" } });
		l.from(r, { rotateX: -95, duration: 4, ease: "elastic.out(1, 0.3)" }),
			l.from(
				s,
				{
					xPercent: -300,
			...
			t.from(i, {
				...
				duration: 2,
				ease: "power3.out",
				scrollTrigger: { trigger: a, start: "top 70%" },
			});
	}
	intro() {
		const e = this.selectors.block(".home-animate__intro-heading"),
		...
		t.from(r, {
			...
			scrollTrigger: { trigger: e, start: "top 70%" },
		});
		const c = t.timeline({
			scrollTrigger: { trigger: this.DOM.intro, start: "top 70%" },
		});
		c.from(s, {
			xPercent: 300,
			rotateZ: 360,
	...
	basicWords() {
		this.selectors
			.block(".home-animate__text-group > span:not([class])")
			.forEach((r) => {
				this.basicWord(r);
			});
	}
	niceAndEasyEasing() {
		const e = this.selectors.nice(".home-animate__text--green-gradient"),
			...
			o = t.timeline({
				scrollTrigger: { trigger: e, ...this.triggerDefaults },
			});
		o.from([e, r], {
			autoAlpha: 0,
			rotateZ: -20,
	...
	superPlugAndPlay() {
		...
		o.from(e, {
			...
			o.from(r, { autoAlpha: 0, duration: 0.7, ease: "power3.out" }, 0.6),
			o.from(
				s,
				{
					autoAlpha: 0,
	...
	asterisk() {
		...
		return (
			...
			r.to(e, { scale: 1.15, duration: 0.3, ease: "power3.inOut" }),
			r.to(e, { scale: 1, duration: 0.3, ease: "power3.inOut" }),
			r
		);
	}
	asteriskScrub() {
	...
	customCurves() {
		...
		t.from(r.chars, {
			...
			t.from(o.chars, {
				...
				ease: a,
				stagger: 0.175,
				scrollTrigger: { trigger: s, ...this.triggerDefaults },
			});
	}
	curvedLineToggle() {
		const e = this.selectors.super(".home-animate__icon--curve"),
			...
			a = { duration: 2, ease: "power3.inOut" },
			i = t.timeline({
				defaults: a,
				scrollTrigger: {
					trigger: e,
	...
	animationSequences() {
		const e = this.selectors.choreograph(".home-animate__text--green"),
			...
			o = { duration: 0.5, ease: "power3.inOut" },
			a = t.timeline({
				defaults: o,
				scrollTrigger: {
		...
		t.set(e, { yPercent: -102.5, xPercent: -52.5 }),
			t.set(r, { xPercent: -140 }),
			t.set(s, { autoAlpha: 0, scale: 0.3 }),
			a.to(e, { yPercent: 0 }),
			a.to(e, { xPercent: 0 }),
			a.to(
				r,
			...
			a.to(s, { autoAlpha: 1, scale: 1, ease: "back.out(2.7)" }, "-=.2");
	}
	snap() {
		const e = this.selectors.snap(".home-animate__diamond"),
			...
			o = { duration: 0.6, ease: "elastic.out(1, 0.75)" },
			a = t.timeline({
				defaults: o,
				scrollTrigger: { trigger: e, ...this.triggerDefaults },
			});
		a.from(e, { autoAlpha: 0 }, 0),
			a.to(
				e,
				{
					keyframes: [
						{ rotationZ: 90, ...o },
						{ rotationZ: 180, delay: 1, ...o },
						{ rotationZ: 270, delay: 1, ...o },
						{ rotationZ: 360, delay: 1, ...o },
					],
					repeat: -1,
					repeatDelay: 1,
				},
			...
			a.fromTo(
				r,
				{ autoAlpha: 0, rotationZ: -60 },
				{ autoAlpha: 1, rotationZ: 0 },
				0
			),
			a.fromTo(
				s[0],
				{ autoAlpha: 0, rotationZ: -60 },
				{ autoAlpha: 1, rotationZ: 0, repeat: -1, repeatDelay: 1 },
				1.6
			),
			a.fromTo(
				s[1],
	...
	createTimelines() {
		t.set(this.DOM.trigger, { autoAlpha: 1 });
		...
		e.add(
			...
			() => {
				...
				(this.scrollTween = r),
					(this.triggerDefaults.containerAnimation =
						this.scrollTween),
					this.intro(),
					this.basicWords(),
			...
			e.add(
				"(prefers-reduced-motion: no-preference) and (max-width: 1239px)",
				() => {
					this.introMobile();
				}
			);
...
class b extends u {
	init() {
		const e = {
			...
			svgSquareLarge: this.block.querySelector(
				"#home-tools-svg-square-large"
			),
			svgCircle: this.block.querySelectorAll(".home-tools-svg-circle"),
			svgCircleSmall: this.block.querySelector(
	...
	setIndexToolsScroll() {
		t.set(this.el.scrollMask, { drawSVG: "0% 100%" });
	}
	playIndexToolsScroll() {
		t.timeline({
			...
			.to(this.el.scrollMask, { drawSVG: "85% 100%" }, "<");
	}
	playIndexToolsSvg() {
		t.timeline({
			defaults: { duration: 1.25, ease: "power3.inOut" },
			...
			.to(this.el.svgCircle, { morphSVG: this.el.svgSquareLarge }, 0)
			.fromTo(
				this.el.svgCircleSmall,
				{ transformOrigin: "80% 20%", scale: 0.3 },
				{ scale: 1 },
				0
			);
	}
	setIndexToolsText() {
		t.set(this.el.textCircleWrapper, { transformOrigin: "0% 0%" }),
			t.set(this.el.textCircle, { transformOrigin: "50% 50%" }),
			t.set(this.el.textSquareWrapper, { transformOrigin: "100% 100%" }),
			t.set(this.el.textSquare, {
				transformOrigin: "50% 50%",
				rotateX: 180,
			}),
			t.set(this.el.textTe, { transformOrigin: "50% 50%", rotateX: 540 });
	}
	playIndexToolsText() {
		const e = t.timeline({
			scrollTrigger: {
		...
		e.to([this.el.textCircleWrapper, this.el.textSquareWrapper], {
			...
			e.to(
				this.el.textTe,
				{ rotateX: 0, duration: 1.5, ease: "Power4.inOut" },
				"<"
			),
			e.to(
				this.el.textXt,
				{ rotateX: 180, duration: 0.75, ease: "Power4.inOut" },
				"-=0.5"
			),
			e.to(
				this.el.textSquare,
				{ rotateX: 0, duration: 0.75, ease: "Power4.inOut" },
				"<"
			),
			e.to(this.el.textXt, {
				rotateX: 0,
			...
			e.to(
				this.el.textSquare,
				{ rotate: 180, duration: 0.75, ease: "Power4.inOut" },
				"<"
			);
	}
	setIndexToolsUi() {
	...
	playIndexToolsUi() {
		...
		t.timeline({
			...
			.to(i, { yPercent: -100 }, e)
			.set(o, { opacity: 1 })
			.to(i, { yPercent: "+=100", xPercent: "+=100" })
			.to(a, { yPercent: "+=100", xPercent: "-=100" }, e)
			.to(s, { yPercent: "-=100", xPercent: "-=100" }, e)
			.to(r, { yPercent: "-=100", xPercent: "+=100" }, e);
	}
	initIndexTools() {
		this.setIndexTools(), this.playIndexTools();
	}
}
class O extends x {
	createBlock(e, r) {
